...
test("handles user input and displays API response", async () => {
  fetch.mockResponseOnce(
    JSON.stringify({ response: "Lucid Motors is a luxury EV brand." }),
  );
  ...
  fireEvent.change(textarea, {
    target: { value: "Tell me about Lucid cars." },
  });
  fireEvent.click(button);

  expect(button).toHaveTextContent("Sending...");
  expect(
    await screen.findByText(/Lucid Motors is a luxury EV brand./i),
  ).toBeInTheDocument();
});
